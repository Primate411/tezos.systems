...
/** The completed-month gold average answers first; its clock is explicit. */
function metalsAnswer(snapshot) {
    const gold = latestHistory(snapshot, 'XAU');
    const histories = metalRows(snapshot).filter(row => normalizeHistoryRows(snapshot, row.id).length).length;
    return gold
        ? `Gold averaged ${formatUsd(gold.value)} per troy ounce in ${formatDate(gold.period, true)}; ${histories} of ${metalRows(snapshot).length} metals have comparable monthly prices.`
        : `${histories} of ${metalRows(snapshot).length} metals have comparable completed-month prices.`;
}

function metalsHeaderMeta(snapshot) {
    const gold = latestHistory(snapshot, 'XAU');
    return `${metalRows(snapshot).length} metals · IMF completed-month history${gold ? ` through ${formatDate(gold.period, true)}` : ''} · USGS annual context`;
}
...
function renderChamber(snapshot) {
    ...
    return `<div class="metals-header market-room-header" data-quiet-key="metals-header">${renderChamberHeader({ room: 'metals-room', strip: ['Tezos.Systems', 'Precious Metals', 'Eight-metal assay'], glyph: 'au', title: 'Precious Metals Chamber', titleId: 'metals-title', chips: `${renderChamberChip(freshness.stale ? 'Last-good receipt' : `${metalRows(snapshot).length} metals`, { tone: freshness.stale ? 'historical' : 'live' })}<span class="metals-freshness market-room-freshness lb-live-pill lb-refresh-pill${freshness.stale ? ' is-stale' : ''}" id="metals-freshness" aria-live="polite">${renderAgeingLabel(freshness.label, snapshot.generatedAt, ageLabel(snapshot.generatedAt))}</span>`, summary: 'Gold, silver and the six platinum-group metals, with gaps kept visible', meta: escapeHtml(metalsHeaderMeta(snapshot)), className: 'chamber-anim-fade' })}${snapshotStatusMarkup(savedSnapshot, lastRefreshError, snapshot.sources)}</div><div class="metals-tabs market-room-tabs chamber-tabs" role="tablist" aria-label="Precious Metals Chamber views">${VIEWS.map((item) => `<button class="metals-tab market-room-tab chamber-tab" id="metals-tab-${item.id}" type="button" role="tab" aria-selected="${item.id === currentView}" aria-controls="metals-view-panel" tabindex="${item.id === currentView ? '0' : '-1'}" data-metals-view="${item.id}">${escapeHtml(item.label)}</button>`).join('')}</div>${renderChamberVerdict({ key: 'metals', state: lastRefreshError || freshness.stale ? 'watch' : 'snapshot', sentence: metalsAnswer(snapshot), note: 'All eight metals belong in the room, but comparable monthly prices cover only the IMF quartet; missing prices stay missing.', receipts: [['Metals', metalRows(snapshot).length], ['Monthly histories', metalRows(snapshot).filter(row => normalizeHistoryRows(snapshot, row.id).length).length]] })}<section class="metals-view-shell market-room-view-shell" id="metals-view-panel" role="tabpanel" aria-labelledby="metals-tab-${view.id}" data-quiet-key="metals-view-panel"><div class="metals-view-head market-room-view-head"><div><h3>${escapeHtml(view.title)}</h3><p>${escapeHtml(view.detail)}</p></div></div><div class="metals-view-content market-room-view-content" id="metals-view-content" data-quiet-key="metals-view-content">${renderView(snapshot)}</div></section>${renderChamberGuide('metals')}<p class="metals-disclaimer">Information only · public-source observations · not investment, custody, legal, redemption, or trading advice.</p>`;
}
