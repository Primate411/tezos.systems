...
function renderChamber(snapshot) {
    ...
    const freshness = freshnessModel(snapshot);
    return `<div class="minerals-header market-room-header" data-quiet-key="minerals-header">${renderChamberHeader({ room: 'minerals-room', strip: ['Tezos.Systems', 'Critical Minerals', 'Strategic materials'], glyph: 'min', title: firstText(snapshot.identity?.title, 'Critical Minerals Chamber'), titleId: 'minerals-title', chips: `${renderChamberChip(freshness.stale ? 'Last-good receipt' : `${mineralRows(snapshot).length} minerals`, { tone: freshness.stale ? 'historical' : 'live' })}<span class="minerals-freshness market-room-freshness lb-live-pill lb-refresh-pill${freshness.stale ? ' is-stale' : ''}" id="minerals-freshness" aria-live="polite">${renderAgeingLabel(freshness.label, snapshot.generatedAt, ageLabel(snapshot.generatedAt))}</span>`, summary: 'The official list, USGS supply, and a bounded monthly market subset', meta: escapeHtml(`${mineralRows(snapshot).length} minerals · ${Object.keys(snapshot.markets?.series || {}).length} monthly series · final 2025 U.S. list`), className: 'chamber-anim-fade' })}${snapshotStatusMarkup(savedSnapshot, lastRefreshError, snapshot.sources)}</div><div class="minerals-tabs market-room-tabs chamber-tabs" role="tablist" aria-label="Critical Minerals Chamber views">${VIEWS.map((item) => `<button class="minerals-tab market-room-tab chamber-tab" id="minerals-tab-${item.id}" type="button" role="tab" aria-selected="${item.id === currentView}" aria-controls="minerals-view-panel" tabindex="${item.id === currentView ? '0' : '-1'}" data-minerals-view="${item.id}">${escapeHtml(item.label)}</button>`).join('')}</div>${renderChamberVerdict({ key: 'minerals', state: lastRefreshError || freshness.stale ? 'watch' : 'snapshot', sentence: `${mineralRows(snapshot).length} critical minerals are on the final 2025 U.S. list; ${Object.keys(snapshot.markets?.series || {}).length} have comparable monthly World Bank prices.`, note: 'Monthly price coverage is deliberately narrower than the atlas; a listed mineral need not have a token or a market quote.', receipts: [['Minerals', mineralRows(snapshot).length], ['Monthly series', Object.keys(snapshot.markets?.series || {}).length]] })}<section class="minerals-view-shell market-room-view-shell" id="minerals-view-panel" role="tabpanel" aria-labelledby="minerals-tab-${view.id}" data-quiet-key="minerals-view-panel"><div class="minerals-view-head market-room-view-head"><div><h3>${escapeHtml(view.title)}</h3><p>${escapeHtml(view.detail)}</p></div></div><div class="minerals-view-content market-room-view-content" id="minerals-view-content" data-quiet-key="minerals-view-content">${renderView(snapshot)}</div></section>${renderChamberGuide('minerals')}<p class="minerals-disclaimer">Information only · public-source observations · no investment, custody, legal, redemption, or execution advice.</p>`;
}
