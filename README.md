...
- CSS tooling uses the direct `clean-css` dependency. The build and pre-commit
  generator share `scripts/lib/css-bundles.mjs`, covering all 17 lazy surface
  bundles, the HEN/site-map/landing/loading stylesheets, and all 15 themes. Minified home/search styles retain synchronous
  Index geometry and shared controls; Anthology reading styles load on intent.
  `node scripts/build-css.mjs --check` verifies generated bytes without writing.
...
Price and API deadlines include response-body parsing, so a stalled read remains
retryable. TzKT pacing has a separate bounded wait before the network deadline.
Source-specific contracts validate statistics, numeric RPC values, timing,
governance, counts, baker catalogs, and prices before caching. Invalid cached
responses are evicted before a fresh attempt. Valid zero values stay zero;
missing values remain unavailable. Dashboard and price storage reads also check
their schema, values, and observation clocks.
...
failures. The bounded API read cache distinguishes headers and response formats
and excludes writes and `no-store` requests. My Tezos deduplication also separates
providers, request bodies, credentials, and retry/deadline policies. Caller abort
detaches that reader without cancelling another reader's shared work. Active
requests have 15-second attempts and a 60-second total budget (30/90 seconds for
archive RPC); retry waits count against that budget. Paused work starts its
budget only when dispatched. Existing provider concurrency limits remain in use.
`tests/request-lifecycle-check.mjs` and `tests/source-payload-check.mjs` exercise
these races, corrupt successes, deadlines, cancellation, and retry recovery.

All eight live widgets validate complete readings before a quiet update. They
retain the last successful reading, its source time, and a stable retry row on
failure; only a confirmed baker HTTP 404 means missing. Hidden responses queue
one catch-up, and updates preserve focus, selection, and page/nested scroll.
Zero staked supply remains zero instead of falling back to another balance.
...
- Network Pulse Chamber with direct `#pulse` and `/pulse/` access. It gathers
  ...
  background refresh. Its high-priority Release Radar reads one explicitly reviewed
...
- My Tezos adaptive personal room with Overview, Baker Signal, Portfolio, Transactions,
  ...
  or signature. Both paths keep data in this browser and explain the seven views
...
The small `js/ui/share.js` facade binds controls and shares seal/APY state; its
image renderer loads on capture intent. Optional calculator, comparison, State
of Tezos, and native explorer controllers initialize when opened. Failed loads
remain retryable, and first actions are retained while their dependencies load.

`tests/lib/static-test-catalog.mjs` declares the standard static gate once.
`npm run test:static` executes that catalog and `npm run test:static:list` lists it;
local testing and hosted workflows use the same entry point. SIGINT/SIGTERM stops
further suites and retries, closes owned resources, writes cancellation receipts,
and bounds cleanup of descendant processes without touching an existing preview.
...
Capital, Critical Minerals, Ecosystem Activity, and Maxis Passport shards use
independent versioned transports under `data/transports/v1/data/`. Decoding
reconstructs the exact expanded bytes before existing source, schema, and hash
checks. Expanded public data and frozen season artifacts remain authoritative.
The generator checks all 67 derivations; scheduled lanes refresh them atomically
with their sources. Only absent rollout files (404/410) fall back to expanded
data. Mutable transports retain the service worker's network-only policy.
